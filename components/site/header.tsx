...
import { ChevronDown, LogIn, Menu, Minus, Plus, UserRound, X } from "lucide-react";
...
function TopBar() {
  ...
  const mins = next ? Math.max(0, Math.round((next.time.getTime() - now.getTime()) / 60000)) : 0;
    ...
    <div className="bg-maroon-dark text-[12px] text-white/85">
      <div className="mx-auto flex h-9 max-w-7xl items-center justify-between gap-3 px-4 md:px-8">
        <p className="flex items-center gap-2 truncate">
          ...
          <span className="hidden sm:inline">— جميع الأسماء والأرقام والبيانات وهمية لأغراض العرض</span>
        </p>
        <div className="flex items-center gap-4">
          {hydrated && (
            <>
              <span className="hidden md:inline">{hijriDate(now, { weekday: "long" })}</span>
              {next && (
                <Link href="/prayer-times" className="hidden items-center gap-1.5 hover:text-gold lg:flex">
...
export function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      <motion.div animate={{ height: scrolled ? 0 : "auto", opacity: scrolled ? 0 : 1 }} className="overflow-hidden">
        <TopBar />
      </motion.div>
      <div
        className={cn(
          "transition-all duration-500 ease-out-expo",
          solid ? "border-b border-gold/30 bg-white/90 shadow-[0_10px_40px_-20px_rgba(2,21,38,.35)] backdrop-blur-xl" : "bg-transparent",
        )}
      >
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-4 md:px-8">
          ...
          <nav className="hidden items-center gap-1 xl:flex">
            {NAV.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  solid ? "text-ink-soft hover:text-green-dark" : "text-white/85 hover:text-white",
                  isActive(item.href) && (solid ? "text-green-dark" : "text-white"),
                )}
              >
                {isActive(item.href) && (
                  <motion.span
                    layoutId="nav-pill"
                    className={cn("absolute inset-0 -z-10 rounded-full", solid ? "bg-green-dark/8" : "bg-white/15")}
            ...
            <div className="relative" onMouseEnter={() => setMore(true)} onMouseLeave={() => setMore(false)}>
              <button
                onClick={() => setMore((v) => !v)}
                className={cn("flex items-center gap-1 rounded-full px-3.5 py-2 text-[15px] font-semibold", solid ? "text-ink-soft" : "text-white/85")}
              >
                المزيد <ChevronDown className={cn("size-4 transition", more && "rotate-180")} />
              </button>
              <AnimatePresence>
                {more && (
                  <motion.div
          ...
          <div className="flex items-center gap-2">
            {person ? (
              <Link
                ...
                className={cn(
                  ...
                  solid ? "bg-green-dark text-white hover:bg-green" : "bg-white/15 text-white backdrop-blur hover:bg-white/25",
                )}
              >
                <span className="grid size-8 place-items-center rounded-xl bg-gold font-display text-ink">{person.firstName[0]}</span>
              ...
              <>
                <Link
                  href="/login"
                  className={cn("hidden items-center gap-1.5 rounded-2xl px-4 py-2.5 text-sm font-bold sm:flex", solid ? "text-green-dark hover:bg-green-dark/5" : "text-white hover:bg-white/10")}
                >
            ...
            <button
              onClick={() => setOpen((v) => !v)}
              className={cn("grid size-11 place-items-center rounded-2xl xl:hidden", solid ? "text-green-dark hover:bg-green-dark/5" : "text-white hover:bg-white/10")}
              aria-label="القائمة"
              aria-expanded={open}
            >
              {open ? <X /> : <Menu />}
          ...
          {open && (
            ...
            >
              <div className="mx-auto grid max-w-7xl gap-1 px-4 pb-6">
                ...
                {!person && (
                  <Link href="/login" className="mt-2 rounded-2xl border-2 border-green-dark/15 px-4 py-3.5 text-center text-lg font-bold text-green-dark">
                    تسجيل الدخول
                  </Link>
                )}
              </div>
