...
import { Building2, CalendarDays, ChevronLeft, ChevronRight, Clock3, Eye, Hash, PenLine } from "lucide-react";
...
import { ARTICLES, getArticle, relatedArticles, type Block } from "@/lib/data/news";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!article) return { title: "منشور غير موجود" };
  return {
    title: article.title,
          ...
          <article className="min-w-0">
            <Reveal>
              <div className="relative -mt-4 aspect-[16/8] overflow-hidden rounded-3xl shadow-[0_30px_70px_-40px_rgba(2,21,38,.7)] ring-1 ring-gold/30 md:-mt-8">
                <Image src={article.image} alt="" fill priority quality={85} sizes="(min-width: 1024px) 860px, 100vw" className="object-cover" />
