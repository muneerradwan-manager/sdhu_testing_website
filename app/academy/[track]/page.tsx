...
import { ChevronLeft, Clock, Layers, ListVideo, PlayCircle, Users } from "lucide-react";
import { PageHero } from "@/components/ui/page-hero";
import { ButtonLink } from "@/components/ui/button";
import { Reveal, SectionHeading, Stagger, StaggerItem } from "@/components/ui/motion";
import { LECTURERS, LESSON_FORMS, TRACKS, arabicCount, getTrack, trackLessons, trackMinutes } from "@/lib/data/academy";
          ...
          {others.map((t) => (
            <StaggerItem key={t.slug}>
              <Link href={`/academy/${t.slug}`} className="group relative flex h-44 flex-col justify-end overflow-hidden rounded-3xl p-4 text-white ring-1 ring-gold/30">
                <Image src={t.image} alt="" fill sizes="(min-width: 1024px) 20vw, 50vw" quality={70} className="-z-20 object-cover transition duration-700 group-hover:scale-110" />
